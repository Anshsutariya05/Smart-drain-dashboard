    ...
    <Card className="animate-slide-in">
      ...
      <CardContent className="space-y-4">
        <div className="flex gap-2">
          ...
          />
          <Button size="sm" onClick={() => fetchWeather(city)} disabled={isLoading}>
            <Search className="h-4 w-4" />
          </Button>
        </div>

        {error && <div className="text-xs text-yellow-600 bg-yellow-50 dark:bg-yellow-900/20 p-2 rounded">{error}</div>}
        ...
        {weather && (
          <div className="space-y-4">
            ...
            <div className="text-center">
              <div className={`text-3xl font-bold ${getDesignAccent()}`}>{weather.temperature}°C</div>
            </div>
            ...
            {drainageImpact && (
              <div className="pt-3 border-t">
                <div className="text-xs text-muted-foreground mb-1">Drainage System Impact:</div>
                <div className={`text-sm font-medium ${drainageImpact.color}`}>{drainageImpact.message}</div>
              </div>
            )}
          </div>
        )}
