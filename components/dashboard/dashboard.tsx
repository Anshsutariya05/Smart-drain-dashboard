...
export default function Dashboard({ user }: DashboardProps) {
  ...
  return (
    <div className={`min-h-screen p-6 ${getDesignClasses()} transition-all duration-500`}>
      <div className="max-w-7xl mx-auto space-y-6">
        ...
        <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
          ...
          <Card className="animate-slide-in" style={{ animationDelay: "0.1s" }}>
            ...
            <CardContent>
              <div className="text-2xl font-bold">
                {latestReading ? `${latestReading.distance.toFixed(1)}cm` : "--"}
              </div>
              <p className="text-xs text-muted-foreground">Distance from sensor</p>
            </CardContent>
          </Card>
          ...
          <Card className="animate-slide-in" style={{ animationDelay: "0.2s" }}>
            ...
            <CardContent>
              <div className="text-2xl font-bold">{latestReading?.motor || "--"}</div>
              <p className="text-xs text-muted-foreground">
                {latestReading?.distance && latestReading.distance < 200 ? "Auto ON (< 200cm)" : "Auto OFF (≥ 200cm)"}
              </p>
            </CardContent>
          </Card>
          ...
          <Card className="animate-slide-in" style={{ animationDelay: "0.3s" }}>
            ...
            <CardContent>
              <div className="text-2xl font-bold">{isConnected ? totalReadings : sensorData.length}</div>
              <p className="text-xs text-muted-foreground">{isConnected ? "From IoT system" : "Local readings"}</p>
            </CardContent>
          </Card>
        </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
          ...
          <div className="space-y-6">
            ...
            <MotorStatus latestReading={latestReading} designVariant={currentDesign} />
          </div>
        </div>
